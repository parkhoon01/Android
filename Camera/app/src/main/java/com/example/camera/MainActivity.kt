...
class MainActivity : AppCompatActivity() {
    ...
    // requestPermission 실행 결과에 대해 처리
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        super.onRequestPermissionsResult(requestCode, permissions, grantResults)
        if(grantResults[0] == PackageManager.PERMISSION_GRANTED && grantResults[1] == PackageManager.PERMISSION_GRANTED){}
        else{
            // 사용자가 permission을 거부하는 경우
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
                if(shouldShowRequestPermissionRationale(Manifest.permission.CAMERA) ||
                    shouldShowRequestPermissionRationale(Manifest.permission.WRITE_EXTERNAL_STORAGE)){
                    val builder = AlertDialog.Builder(this)
                    builder.setMessage("This permission is very important the app.").setTitle("Request Permisiion")
                    builder.setPositiveButton("OK"){
                        dialog, id -> requestPermissions(arrayOf(Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE), 0)
                    }
                }
                else{}
            }
